...
function load_img() {
  var img_url_zero = `https://s3.us-west-2.amazonaws.com/rfmapping/SGD/zero_initialized/${model_name}/${layer}/${unit_id}.png`;
  var img_url_top_patch = `https://s3.us-west-2.amazonaws.com/rfmapping/SGD/top_patch_initialized/${model_name}/${layer}/${unit_id}.png`;
  var img_url_top_patch_original = `https://s3.us-west-2.amazonaws.com/rfmapping/top_patch/${model_name}/${layer}/${unit_id}.png`;

  var html_template= [
    '<h3>Zero-initialized: </h3>',
    '<h3>Top-patch-initialized: </h3>',
    '<h3>Top patch (original): </h3>',
    '<img style="width:300px; margin: 0 auto;" src="' + img_url_zero + '"/>',
    '<img style="width:300px; margin: 0 auto;" src="' + img_url_top_patch + '"/>',
    '<img style="width:300px; margin: 0 auto;" src="' + img_url_top_patch_original + '"/>'
  ];
  document.getElementById('img_div').innerHTML = getHtml(html_template);
};
load_img();
